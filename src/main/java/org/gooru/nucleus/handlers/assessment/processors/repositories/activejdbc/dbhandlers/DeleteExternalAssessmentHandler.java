...
class DeleteExternalAssessmentHandler implements DBHandler {
  private static final Logger LOGGER = LoggerFactory.getLogger(DeleteExternalAssessmentHandler.class);
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    // Fetch the assessment where type is external_assessment and it is not deleted already and id is specified id

    LazyList<AJEntityAssessment> assessments =
      AJEntityAssessment.findBySQL(AJEntityAssessment.AUTHORIZER_QUERY, AJEntityAssessment.ASSESSMENT_EXTERNAL, context.assessmentId(), false);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    return new ExecutionResult<>(MessageResponseFactory
      .createNoContentResponse(RESOURCE_BUNDLE.getString("deleted"), EventBuilderFactory.getDeleteExAssessmentEventBuilder(context.assessmentId())),
