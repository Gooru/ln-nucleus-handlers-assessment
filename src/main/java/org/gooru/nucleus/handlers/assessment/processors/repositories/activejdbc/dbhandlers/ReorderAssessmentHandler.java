...
class ReorderAssessmentHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> validateRequest() {
        ...
        try {
            List idList =
                Base.firstColumn(AJEntityQuestion.QUESTIONS_FOR_ASSESSMENT_QUERY, this.context.assessmentId());
            ...
            if (idList.size() != input.size()) {
                return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
                    RESOURCE_BUNDLE.getString("question.count.mismatch")), ExecutionResult.ExecutionStatus.FAILED);
            }
            for (Object entry : input) {
                String payloadId = ((JsonObject) entry).getString(AJEntityAssessment.ID);
                if (!idList.contains(UUID.fromString(payloadId))) {
                    return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
                        RESOURCE_BUNDLE.getString("missing.questions")), ExecutionResult.ExecutionStatus.FAILED);
                }
            }
        } catch (DBException | ClassCastException e) {
            return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
                RESOURCE_BUNDLE.getString("incorrect.payload.data.types")), ExecutionResult.ExecutionStatus.FAILED);
        }
