...
public final class EventBuilderFactory {
  ...
  public static EventBuilder getDeleteExAssessmentEventBuilder(String assessmentId) {
    return () -> new JsonObject().put(EVENT_NAME, EVT_EX_ASSESSMENT_DELETE).put(EVENT_BODY, new JsonObject().put(ASSESSMENT_ID, assessmentId));
  }

  public static EventBuilder getCreateExAssessmentEventBuilder(String assessmentId) {
    return () -> new JsonObject().put(EVENT_NAME, EVT_EX_ASSESSMENT_CREATE).put(EVENT_BODY, new JsonObject().put(ASSESSMENT_ID, assessmentId));
  }

  public static EventBuilder getUpdateExAssessmentEventBuilder(String assessmentId) {
    return () -> new JsonObject().put(EVENT_NAME, EVT_EX_ASSESSMENT_UPDATE).put(EVENT_BODY, new JsonObject().put(ASSESSMENT_ID, assessmentId));
  }
}
