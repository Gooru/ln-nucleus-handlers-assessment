...
class AddQuestionToAssessmentHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> checkSanity() {
        ...
        if (context.assessmentId() == null || context.assessmentId().isEmpty() || context.questionId() == null
            || context.questionId().isEmpty()) {
            LOGGER.warn("Missing assessment/question id");
            return new ExecutionResult<>(
                MessageResponseFactory
                    .createInvalidRequestResponse(RESOURCE_BUNDLE.getString("missing.assessment.question.id")),
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        try {
            ...
            long count = Base.exec(AJEntityQuestion.ADD_QUESTION_QUERY, this.context.assessmentId(),
                ...
                this.assessment.getString(AJEntityAssessment.LESSON_ID), this.context.userId(), sequenceId,
            ...
            if (count == 1) {
                ...
                return new ExecutionResult<>(
                    MessageResponseFactory
                        .createInternalErrorResponse(RESOURCE_BUNDLE.getString("question.not.exists.or.not.available")),
                    ExecutionResult.ExecutionStatus.FAILED);
            } else {
                LOGGER.error("Something is wrong. Adding question '{}' to assessment '{}' updated '{}' rows",
                    this.context.questionId(), this.context.assessmentId(), count);
                return new ExecutionResult<>(
                    MessageResponseFactory.createInternalErrorResponse(RESOURCE_BUNDLE.getString("question.add.fail")),
                    ExecutionResult.ExecutionStatus.FAILED);
            }
    ...
    private ExecutionResult<MessageResponse> updateGrading() {
        ...
        if (!currentGrading.equalsIgnoreCase(AJEntityAssessment.GRADING_TYPE_TEACHER)) {
            try {
                long count = Base.count(AJEntityQuestion.TABLE_QUESTION, AJEntityQuestion.OPEN_ENDED_QUESTION_FILTER,
