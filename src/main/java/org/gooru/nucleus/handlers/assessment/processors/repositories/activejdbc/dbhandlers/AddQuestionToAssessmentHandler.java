...
class AddQuestionToAssessmentHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> checkSanity() {
    ...
    if (context.assessmentId() == null || context.assessmentId().isEmpty()
        ...
        || context.questionId().isEmpty()) {
      LOGGER.warn("Missing assessment/question id");
      return new ExecutionResult<>(
          MessageResponseFactory
              .createInvalidRequestResponse(
                  RESOURCE_BUNDLE.getString("missing.assessment.question.id")),
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    return AuthorizerBuilder.buildAddQuestionToAssessmentAuthorizer(this.context)
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    try {
      Object sequence = Base
          .firstCell(AJEntityQuestion.MAX_QUESTION_SEQUENCE_QUERY, this.context.assessmentId());
      int sequenceId = 1;
      if (sequence != null) {
      ...
      if (count == 1) {
        ...
        LOGGER.error(
            "Question '{}' add to assessment '{}' failed as question is not available or non existent",
            this.context.questionId(), this.context.assessmentId());
        return new ExecutionResult<>(
            MessageResponseFactory
                .createInternalErrorResponse(
                    RESOURCE_BUNDLE.getString("question.not.exists.or.not.available")),
            ExecutionResult.ExecutionStatus.FAILED);
      } else {
        LOGGER
            .error("Something is wrong. Adding question '{}' to assessment '{}' updated '{}' rows",
                this.context.questionId(), this.context.assessmentId(), count);
        return new ExecutionResult<>(
            MessageResponseFactory
                .createInternalErrorResponse(RESOURCE_BUNDLE.getString("question.add.fail")),
            ExecutionResult.ExecutionStatus.FAILED);
      }

    } catch (DBException e) {
      LOGGER.error("Not able to add question '{}' to assessment '{}'", this.context.questionId(),
  ...
  private ExecutionResult<MessageResponse> updateGrading() {
    ...
    if (!currentGrading.equalsIgnoreCase(AJEntityAssessment.GRADING_TYPE_TEACHER)) {
      try {
        long count = Base
            .count(AJEntityQuestion.TABLE_QUESTION, AJEntityQuestion.OPEN_ENDED_QUESTION_FILTER,
        ...
        LOGGER
            .error("Assessment '{}' grading type change lookup failed", this.context.assessmentId(),
                e);
        return new ExecutionResult<>(
            MessageResponseFactory
                .createInternalErrorResponse(RESOURCE_BUNDLE.getString("error.from.store")),
